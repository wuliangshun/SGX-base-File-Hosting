...
static sgx_status_t SGX_CDECL Enclave_ocall_open(void* pms)
{
	...
	ms->ms_retval = ocall_open((const char*)ms->ms_path);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_create(void* pms)
{
	ms_ocall_create_t* ms = SGX_CAST(ms_ocall_create_t*, pms);
	ms->ms_retval = ocall_create((const char*)ms->ms_path);
...
static sgx_status_t SGX_CDECL Enclave_ocall_read(void* pms)
{
	...
	ms->ms_retval = ocall_read(ms->ms_fd, ms->ms_buf, ms->ms_size);
...
static sgx_status_t SGX_CDECL Enclave_ocall_write(void* pms)
{
	...
	ms->ms_retval = ocall_write(ms->ms_fd, (const void*)ms->ms_buf, ms->ms_size);
...
static sgx_status_t SGX_CDECL Enclave_ocall_print(void* pms)
{
	...
	ocall_print((const char*)ms->ms_msg);
...
{
	sgx_status_t status;
	ms_ecall_encrypt_file_t ms;
	ms.ms_input_file = (char*)input_file;
	ms.ms_input_file_len = input_file ? strlen(input_file) + 1 : 0;
	ms.ms_output_file = (char*)output_file;
